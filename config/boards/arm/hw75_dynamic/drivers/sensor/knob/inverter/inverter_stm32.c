...
static void inverter_stm32_set_powers(const struct device *dev, float a, float b, float c)
{
	...
	__HAL_TIM_SET_COMPARE(&data->th, timer_channels[config->pwm_channels[0]],
			      config->pwm_period * a);

	__HAL_TIM_SET_COMPARE(&data->th, timer_channels[config->pwm_channels[1]],
			      config->pwm_period * b);

	__HAL_TIM_SET_COMPARE(&data->th, timer_channels[config->pwm_channels[2]],
			      config->pwm_period * c);
}
...
static int inverter_stm32_init(const struct device *dev)
{
	...
	for (int i = 0; i < ARRAY_SIZE(config->pwm_channels); i++) {
		if (HAL_TIM_PWM_ConfigChannel(&data->th, &oc,
					      timer_channels[config->pwm_channels[i]]) != HAL_OK) {
